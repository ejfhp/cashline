...
func TestIsValidKey(t *testing.T) {
	tooSmall := "0"
	tooBig := "115792089237316195423570985008687907852837564279074904382605163141518161494338"
	ok := "1157920892373161954235709850086879078528375642790749043826051631415181614943"
	bn := new(big.Int)
	bn.SetString(tooSmall, 10)
	if isValidKey(bn) == true {
		t.Errorf("failed to recognize key val too small: %s", tooSmall)
	}
	bn.SetString(tooBig, 10)
	if isValidKey(bn) == true {
		t.Errorf("failed to recognize key val too big: %s", tooBig)
	}
	bn.SetString(ok, 10)
	if isValidKey(bn) != true {
		t.Errorf("failed to recognize legit key val: %s", ok)
	}
}
...
func TestFromDiceSequenceRandom(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < diceSeqRequiredLength; j++ {
			c := int(math.Round(random.Float64()*5.0) + 1)
